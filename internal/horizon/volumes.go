...
// DashboardEnabledFinalPath is where dashboard_setup's config_dashboard
...
//
// Unlike the theme directory, this EmptyDir is NOT seeded from the image and
// cannot mirror the theme pattern: dashboard_setup *writes* its output here,
// so the EmptyDir must be mounted at this final path (in both the
// dashboard-setup init container and the main container) for those writes to
// be shared -- which necessarily shadows the image's own directory. That's
// safe: the image ships only an empty __init__.py here (all core panels live
// in the sibling openstack_dashboard/enabled/, which is never mounted), and
// config_dashboard rebuilds local/enabled/ from scratch off the ENABLE_* env
// vars, reading each panel from its own service package rather than from any
